...
test_df2 <- data.frame(data = c(NA,7,1,NA,3,3,5.5,6,NA, 3, 1), group = c(2,1,1,1,2,2,3,3,3,1,2))
test_df2$group <- factor(test_df2$group)

result_NA <- c(5.944444, 1.166667, 2.833333, 2.833333, 4.277778, 5.166667, 2.833333, 1.166667)
...
test_that("function psrank missing values", {
  ...
  expect_equivalent(pseudorank::psrank(test_df2$data, test_df2$group, na.last = TRUE), result_TRUE, tolerance=1e-04)
  expect_equivalent(pseudorank::psrank(test_df2$data, test_df2$group, na.last = FALSE), result_FALSE, tolerance=1e-04)
  expect_equivalent(pseudorank::psrank(test_df2$data, test_df2$group, na.last = NA), result_NA, tolerance=1e-04)
})